...
- **Page access — allowlist, not blocklist**: `server.js`'s page-redirect middleware (previously a single hardcoded `client`→`/stock` check) is now a `ROLE_PAGE_ALLOWLIST` map: `client` → `['/stock']` (unchanged), `gelco_worker` → `['/inward', '/outward']`, `gelco_manager` → `['/', '/inward', '/outward', '/transfer', '/requests', '/gelco-docs']`. Anything not in a role's list redirects to `/inward` (or `/stock` for `client`). An allowlist means any future new page is safe-by-default for these two roles without remembering to add it to a blocklist.
...
- **`gelco_manager` is an approver, scoped to Gelco-only requests**: added to `APPROVER_ROLES` in `inward.js`/`outward.js`/`transfer.js`/`requests.js`, so their own actions execute immediately and `requireApprover` lets them into `/api/requests`. The actual restriction is a scoping predicate (`isGelcoRelevant(type, payload)` in `requests.js`) checked in `GET /`, `GET /count`, `POST /:id/approve`, `POST /:id/reject`, and `POST /:id/edit-approve` — for inward/outward it checks `payload.store_code === 'secondary'`; for transfers it checks `payload.from_store === 'secondary' || payload.to_store === 'secondary'`. This is why `routes/transfer.js`'s `POST /` now looks up and stores `from_store` in the pending-request payload for ALL roles (previously it wasn't recorded at submission time at all, only derivable at execution time) — without it, `gelco_manager` couldn't tell whether a pending transfer involved Gelco before approving it. `gelco_worker` gets an explicit `403` on `GET /api/requests` and `GET /count` — they're not an approver and have no legitimate reason to read pending-request data (the pre-existing `user`/`client` exposure on these same endpoints is a separate, un-touched gap).
- **Daily gate**: `gelco_manager`/`gelco_worker` must clear a daily "approve yesterday's Gelco outward summary" gate before they can write anything. See `routes/daily-gate.js` in §4 and `utils/dailyGate.js` in §5 for the mechanism; enforced both as a frontend blocking overlay (`injectDailyGateOverlay()` in `public/js/app.js`, §6) and as a real server-side backstop on the same write endpoints listed above (`isGateApprovedToday('secondary')` checked before any mutation). `admin`/`manager` are exempt (always read as approved).
...
- `GET /recent?limit&offset&store&date_from&date_to` — **response shape is now `{rows, total}`**, not a bare array (changed for real server-side pagination — previously the client always requested `?limit=500` and paginated in-browser). `store` optionally filters by `store_code`; `date_from`/`date_to` (new) filter by `outward_date` using explicit `'YYYY-MM-DD 00:00:00'`/`'YYYY-MM-DD 23:59:59'` string bounds (never SQLite `date()` — see the IST note in §2/§5), added for the Notifications page and reused by the daily-gate summary logic's pattern. `store` is forced to `'secondary'` server-side for `gelco_manager`/`gelco_worker` regardless of the query param.
